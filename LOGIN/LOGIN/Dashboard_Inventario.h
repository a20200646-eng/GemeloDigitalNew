...
namespace LOGIN {
	...
	public:
		Dashboard_Inventario(void)
		{
			...
			ctrlEstacion = gcnew EstacionTrabajoController();
...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->ClientSize = System::Drawing::Size(809, 691);
		...
		CargarLineaEnsamblaje();
		   ...
		   void CargarKPIs()
		   {
			   ...
			   for each (PanelLateralModel ^ p in ctrlPanel->obtenerTodos())
				   if (p->Estado == EstadoPieza::DISPONIBLE) panelesDisp++;
			   label6->Text = panelesDisp.ToString();

			   // Techos disponibles
			   int techosDisp = 0;
			   for each (EstructuraTechoModel ^ t in ctrlTecho->obtenerTodos())
				   if (t->Estado == EstadoPieza::DISPONIBLE) techosDisp++;
			   label8->Text = techosDisp.ToString();
		   ...
		   void CargarStockPorEstado()
		   {
			   dataGridView1->Rows->Clear();
			   dataGridView1->ReadOnly = true;

			   // Contar por estado para cada tipo de pieza
			   int panelDisp = 0, panelProc = 0, panelEns = 0, panelDef = 0;
			   for each (PanelLateralModel ^ p in ctrlPanel->obtenerTodos())
			   {
				   if (p->Estado == EstadoPieza::DISPONIBLE)  panelDisp++;
				   else if (p->Estado == EstadoPieza::EN_PROCESO)  panelProc++;
				   else if (p->Estado == EstadoPieza::ENSAMBLADA)  panelEns++;
				   else if (p->Estado == EstadoPieza::DEFECTUOSA)  panelDef++;
			   }
			   dataGridView1->Rows->Add("Panel Lateral",
				   panelDisp, panelProc, panelEns, panelDef);

			   int techoDisp = 0, techoProc = 0, techoEns = 0, techoDef = 0;
			   for each (EstructuraTechoModel ^ t in ctrlTecho->obtenerTodos())
			   {
				   if (t->Estado == EstadoPieza::DISPONIBLE)  techoDisp++;
				   else if (t->Estado == EstadoPieza::EN_PROCESO)  techoProc++;
				   else if (t->Estado == EstadoPieza::ENSAMBLADA)  techoEns++;
				   else if (t->Estado == EstadoPieza::DEFECTUOSA)  techoDef++;
			   }
			   dataGridView1->Rows->Add("Estructura Techo",
				   techoDisp, techoProc, techoEns, techoDef);
		   }
		   ...
		   void CargarLineaEnsamblaje()
		   {
			   ...
			   // Mostrar datos de la primera línea activa
			   LineaEnsamblajeModel^ l = lineas[0];
			   label17->Text = l->Id;
			   label18->Text = l->IndiceActual.ToString();
			   label19->Text = l->SecuenciaAprobada ? "APROBADA" : "PENDIENTE aprob.";
			   label20->Text = l->ColaPiezas->Count.ToString();

			   // Color secuencia
			   label19->ForeColor = l->SecuenciaAprobada
				   ? Color::FromArgb(0, 200, 100)       // verde
				   : Color::FromArgb(230, 160, 0);      // naranja
		   }
